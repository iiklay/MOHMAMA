...
client.on('message', message => {
    if (message.content === "+help") {
        let pages = [`**👑 Owner Commands      
                    ...
                    +cc| لانشاء الوان
                    +cr | لانشاء رومات
                    +clear | لمسح الرسائل**`,`**🎶 Music Commands
...
if (message.content.startsWith(prefix + 'set-name')) {
  client.user.setUsername(argresult).then
      message.channel.sendMessage(`**${argresult}** : تم بنجاح تغيير الاسم :white_check_mark:`)
  return message.reply("**تم تغيير الاسم البوت بنجاح :white_check_mark:**");
} else
if (message.content.startsWith(prefix + 'set-pic')) {
  client.user.setAvatar(argresult);
    message.channel.sendMessage(`**${argresult}** : تم تغيير صورة البوت بنجاح :white_check_mark:`);
...
client.on("message", message => {
  ...
  if (message.mentions.users.size < 1) return message.reply('** You Have To Mentionً **');
...
if(message.content.startsWith(prefix + 'move')) {
 ...
 return message.channel.send("``لاستخدام الأمر اكتب هذه الأمر : " +prefix+ "move @khaled``")
}
...
const type = require('./gameselderbot/ryd.json');
        ...
        console.log(`[Typing] ${collected.first().author} typed the word.`);
          ...
          .catch(collected => {
              ...
              client.on('message', message => {
                ...
                const type = require('./gameselderbot/RKB.json');
                const item = type[Math.floor(Math.random() * type.length)];
                const filter = response => {
                    return item.answers.some(answer => answer.toLowerCase() === response.content.toLowerCase());
                };
            ...
            const type = require('./gameselderbot/SHAKLEB.json');
            const item = type[Math.floor(Math.random() * type.length)];
            const filter = response => {
                return item.answers.some(answer => answer.toLowerCase() === response.content.toLowerCase());
            };
